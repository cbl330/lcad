...
(function ($) {
	$(function () {
		...
		var soliloquy_append_and_hide = function (e) {
			e.preventDefault();
			$('.soliloquy-default-ui .selected').removeClass(
				'details selected',
			);
		...
		$(document).on(
			...
			function (e) {
				...
				if (
					$(soliloquy_trigger_target).hasClass(
						'soliloquy-choose-slider',
					)
				) {
					wp.media.editor.insert(
						'[soliloquy id="' +
							$('.soliloquy-default-ui .selected').data(
								'soliloquy-id',
							) +
							'"]',
					);
					...
					var req_data = {
						...
						post_id: $(
							'.soliloquy-default-ui:first .selected',
						).data('soliloquy-id'),
					};
					$.post(
						ajaxurl,
