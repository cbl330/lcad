...
if ( ! class_exists( 'Soliloquy_Metaboxes' ) ) :
	...
	class Soliloquy_Metaboxes {
		...
		public function fix_plugin_js_conflicts() {
			...
			wp_dequeue_script( 'ngg-igw' );
			wp_dequeue_script( 'revslider-tinymce-shortcode-script' );
			wp_dequeue_script( 'unite_admin' );
			wp_dequeue_script( 'sa-slider-admin-script' );
		...
		public function add_meta_boxes() {
			...
			if ( ! is_object( $post ) || empty( $post->post_type ) || 'soliloquy' !== $post->post_type ) {
				return;
			}
...
endforeach;
		...
		public function get_soliloquy_tab_nav() {
			...
			$tabs = array(
				...
				'config' => esc_attr__( 'Configuration', 'soliloquy' ),
				 ...
				 <table class="form-table">
					 ...
					 <tbody>
						 ...
						 <tr id="soliloquy-config-slider-size-box">
							 ...
							 <td>
								 <input id="soliloquy-config-slider-width" type="number" name="_soliloquy[slider_width]" value="<?php echo intval( $this->get_config( 'slider_width', $this->get_config_default( 'slider_width' ) ) ); ?>" /> &#215; <input id="soliloquy-config-slider-height" type="number" name="_soliloquy[slider_height]" value="<?php echo absint( $this->get_config( 'slider_height', $this->get_config_default( 'slider_height' ) ) ); ?>" /> <span class="soliloquy-unit"><?php esc_html_e( 'px', 'soliloquy' ); ?></span>
						...
						<?php
						if ( ( class_exists( 'Soliloquy_Defaults' ) || class_exists( 'Soliloquy_Dynamic' ) )  && isset( $post->ID ) ) :
						...
						<!-- Sorting -->
						<tr id="soliloquy-config-sorting-box">
							<th scope="row">
								<label for="soliloquy-config-sorting"><?php _e( 'Sorting', 'soliloquy' ); ?></label>
							</th>
							<td>
								<div class="soliloquy-select">
									<select id="soliloquy-config-slide-sort" name="_soliloquy[sort_order]" class="soliloquy-chosen" data-soliloquy-chosen-options='{ "disable_search":"true", "width": "100%" }'>
								...
								<p class="description"><?php _e( 'Choose the sort order for your photos.', 'soliloquy' ); ?></p>
							</td>
						</tr>
							<?php endif;
							endif;
					   ...
					   <?php
						...
						if ( $post->ID === $dynamic || $post->ID === $default ) {
						 ...
						 <tr id="soliloquy-config-enable-link-box">
							 <th scope="row">
								 <label for="soliloquy-config-enable-link"><?php esc_html_e( 'Enable Link?', 'soliloquy' ); ?></label>
							 </th>
							 <td>
								 <input id="soliloquy-config-enable-link" type="checkbox" name="_soliloquy[enable_link]" value="<?php echo $this->get_config( 'enable_link', $this->get_config_default( 'enable_link' ) ); ?>" <?php checked( $this->get_config( 'enable_link', $this->get_config_default( 'enable_link' ) ), 1 ); ?> />
								 <span class="description"><?php esc_html_e( 'Enables a link for each photo in a slide. Lightbox Addon will override this setting.', 'soliloquy' ); ?></span>
							 </td>
						 </tr>
							 ...
							 <td>
								...
								$import_url = 'auto-draft' == $post->post_status ? add_query_arg(
									array(
										'post'   => $post->ID,
										'action' => 'edit',
										'soliloquy-imported' => true,
									),
									admin_url( 'post.php' )
								) : add_query_arg( 'soliloquy-imported', true );
		...
		public function save_meta_boxes( $post_id, $post ) {
			...
			$settings['config']['enable_link']      = isset( $_POST['_soliloquy']['enable_link'] ) ? 1 : 0;
		...
		public function get_slider_image( $id, $data, $post_id = 0 ) {
			...
			$json = wp_json_encode( $data, JSON_HEX_APOS );
