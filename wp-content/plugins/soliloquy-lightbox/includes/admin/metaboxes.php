...
class Soliloquy_Lightbox_Mextaboxes {
	...
	/**
	 * Helper Method to display mobile tab.
	 *
	 * @param object $post Post Object.
	 * @return void
	 */
	public function tab_mobile( $post ) {
		...
		<tr id="soliloquy-config-mobile-lightbox-box">
			...
			<td>
				<input id="soliloquy-config-mobile-lightbox" type="checkbox" name="_soliloquy[mobile_lightbox]" value="<?php echo esc_attr( $instance->get_config( 'mobile_lightbox', $instance->get_config_default( 'mobile_lightbox' ) ) ); ?>" <?php checked( $instance->get_config( 'mobile_lightbox', $instance->get_config_default( 'mobile_lightbox' ) ), 1 ); ?> />
			...
			<table class="form-table">
				<tbody>
						...
						<td>
							...
							<select id="soliloquy-config-lightbox-theme" name="_soliloquy[lightbox_theme]" class="soliloquy-chosen" data-soliloquy-chosen-options='{ "disable_search":"true", "width": "100%" }'>
								<?php foreach ( (array) $common->lightbox_themes() as $i => $data ) : ?>
									<option value="<?php echo esc_attr( $data['value'] ); ?>"<?php selected( $data['value'], $instance->get_config( 'lightbox_theme', $instance->get_config_default( 'lightbox_theme' ) ) ); ?>><?php echo esc_html( $data['name'] ); ?></option>
								<?php endforeach; ?>
							</select>
							</div>
					...
					<tr id="soliloquy-config-supersize-box">
						<th scope="row">
							<label for="soliloquy-config-supersize"><?php esc_html_e( 'Enable Lightbox Supersize?', 'soliloquy-supersize' ); ?></label>
						</th>
						<td>
							...
							<span class="description"><?php esc_html_e( 'Enables or disables supersize mode for the lightbox.', 'soliloquy-supersize' ); ?></span>
						</td>
					</tr>
	...
	public function meta( $attach_id, $data, $post_id ) {
		...
		<label class="setting">
			...
			<input id="soliloquy-lightbox-enable-<?php echo sanitize_html_class( $attach_id ); ?>" class="soliloquy-lightbox-enable" type="checkbox" name="_soliloquy[lightbox_enable]" data-soliloquy-meta="lightbox_enable" value="<?php echo $instance->get_meta( 'lightbox_enable', $attach_id, $instance->get_meta_default( 'lightbox_enable', $attach_id ) ); ?>"<?php checked( $instance->get_meta( 'lightbox_enable', $attach_id, $instance->get_meta_default( 'lightbox_enable', $attach_id ) ), 1 ); ?> />
		</label>
		<?php
	...
	public function save( $settings, $post_id ) {
		...
		$settings['config']['lightbox_theme']      = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_soliloquy']['lightbox_theme'] ) ) );
		$settings['config']['lightbox_title']      = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_soliloquy']['lightbox_title'] ) ) );
		...
		$settings['config']['lightbox_transition'] = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_soliloquy']['lightbox_transition'] ) ) );
		...
		$settings['config']['lightbox_tposition']  = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_soliloquy']['lightbox_tposition'] ) ) );
	 ...
	 * @return array $settings Amended array of settings to be saved.
	 */
	public function save_meta( $settings, $meta, $attach_id, $post_id ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param array $args    Array of settings to be saved.
	 * @param int   $post_id The current post ID.
	 */
		...
		if ( $images ) {
			...
			foreach ( $images as $id => $item ) {
				...
				if ( is_wp_error( $cropped_image ) ) {
					...
					if ( defined( 'SOLILOQUY_CROP_DEBUG' ) && SOLILOQUY_CROP_DEBUG && defined( 'WP_DEBUG' ) && WP_DEBUG ) {

						// Only displayed during debug.
