 ...
 * @since 2.3.0
 * @package SoliloquyWP
 * @subpackage Soliloquy Thumbnails
...
class Soliloquy_Lightbox_Shortcode {
	...
	/**
	 * Helper method to init shortcode.
	 *
	...
	public function load( $data ) {
		...
		foreach ( (array) $common->lightbox_themes() as $array => $theme_data ) {

			if ( $instance->get_config( 'lightbox_theme', $data) !== $theme_data['value'] || 'base' === $theme_data['value'] ) {
				continue;
			}

			$theme = $instance->get_config( 'lightbox_theme', $data );

			wp_enqueue_style( $this->base->plugin_slug . $theme . '-theme', plugins_url( 'themes/' . $theme . '/style.css', $theme_data['file'] ), array( $this->base->plugin_slug . '-style' ), $this->base->version );
			break;
		}
	 ...
	 */
	public function infuse( $item, $id, $data, $i ) {
		...
		if ( $instance->get_config( 'lightbox_thumbs', $data ) && empty( $item['lb_thumb'] ) ) {
			...
			$item_id = ( isset( $item['id'] ) ? $item['id'] : $id );

			$item['lb_thumb'] = $instance->get_image_src( $item_id, $item, $data, 'lightbox' );
		}
	 ...
	 */
	public function lightbox_attr( $attr, $id, $item, $data, $i ) {
	 ...
	 * @param array $data Array of slider data.
	 */
	public function lightbox_js( $data ) {
		...
		$(document).off('click.soliloquyYouTube<?php echo sanitize_html_class( $data['id'] ); ?> click.soliloquyVimeo<?php echo sanitize_html_class( $data['id'] ); ?> click.soliloquyWistia<?php echo sanitize_html_class( $data['id'] ); ?> click.soliloquyLocal<?php echo sanitize_html_class( $data['id'] ); ?>');
		<?php endif; ?>
		...
		soliloquy_lightbox['<?php echo sanitize_html_class( $data['id'] ); ?>'] = $('#soliloquy-<?php echo sanitize_html_class( $data['id'] ); ?> .soliloquy-link').soliloquybox({
			<?php do_action( 'soliloquy_lightbox_api_config_start', $data ); ?>
			<?php if ( ! $instance->get_config( 'lightbox_keyboard', $data ) ) : ?>
			keys: 0,
			<?php endif; ?>
			scrolling: 'no',
			arrows: <?php echo esc_js( $instance->get_config( 'lightbox_arrows', $data ) ); ?>,
			aspectRatio: <?php echo esc_js( $instance->get_config( 'lightbox_aspect', $data ) ); ?>,
			loop: <?php echo esc_js( $instance->get_config( 'lightbox_loop', $data ) ); ?>,
			mouseWheel: <?php echo esc_js( $instance->get_config( 'lightbox_mousewheel', $data ) ); ?>,
			preload: 1,
			nextEffect: '<?php echo esc_js( $instance->get_config( 'lightbox_transition', $data ) ); ?>',
			prevEffect: '<?php echo esc_js( $instance->get_config( 'lightbox_transition', $data ) ); ?>',
			tpl: {
				wrap     : '<div class="soliloquybox-wrap" tabIndex="-1"><div class="soliloquybox-skin soliloquybox-theme-<?php echo sanitize_html_class( $instance->get_config( 'lightbox_theme', $data ) ); ?>"><div class="soliloquybox-outer"><div class="soliloquybox-inner"></div></div></div></div>',
			...
			helpers: {
				...
				title: {
					...
					type: '<?php echo esc_js( $instance->get_config( 'lightbox_title', $data ) ); ?>'
				},
				video: {
					autoplay: 0,
					playpause: 1,
				...
				thumbs: {
					width: <?php echo esc_js( $instance->get_config( 'lightbox_twidth', $data ) ); ?>,
					height: <?php echo esc_js( $instance->get_config( 'lightbox_theight', $data ) ); ?>,
					source: function(current) {
						return $(current.element).data('thumbnail');
					},
					position: '<?php echo esc_js( $instance->get_config( 'lightbox_tposition', $data ) ); ?>'
				}
				<?php endif; ?>
			},
			...
			beforeShow: function(){
				...
				soliloquy_slider['<?php echo sanitize_html_class( $data['id'] ); ?>'].stopAuto();
			...
			afterClose: function(){
				...
				<?php if ( $instance->get_config( 'auto', $data ) ) { ?>

				soliloquy_slider['<?php echo sanitize_html_class( $data['id'] ); ?>'].startAuto();
				<?php } ?>
	 ...
	 */
	public function get_thumbnail( $image, $id, $item, $data, $slider_id ) {
	...
	public function js_clone( $data ) {
		...
		$('#soliloquy-container-<?php echo sanitize_html_class( $data['id'] ); ?>').find('.soliloquy-clone > a').removeAttr('rel');
		$('#soliloquy-container-<?php echo sanitize_html_class( $data['id'] ); ?>').find('.soliloquy-clone > a').removeAttr('data-soliloquybox-group');
	 ...
	 */
	public function lightbox_theme( $stylesheets, $data ) {
